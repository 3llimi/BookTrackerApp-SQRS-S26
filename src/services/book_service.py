...
def create_book(db: Session, data: BookCreate, user_id: int) -> Book:
    """Create a new book for the user. Raises 409 if a book with the same ISBN already exists for the user."""
    # 409 if ISBN already exists
    if data.isbn:
        existing_book = db.query(Book).filter_by(isbn=data.isbn, user_id=user_id).first()
        if existing_book:
