...
def search_books(db, user_id, q=None, genre=None, author=None,
                 status=None, sort="created_at", order="desc",
                 limit=10, offset=0):
    ...
    if genre:
        query = query.filter(Book.genre.ilike(f"%{genre}%"))  # case-insensitive per spec
