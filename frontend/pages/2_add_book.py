...
def merge_openlibrary_import_data(
    ...
    merged = {
        ...
        "isbn": (detail.get("isbn") or fallback_isbn or result.get("isbn") or "").strip() or None,
        "cover_url": (detail.get("cover_url") or result.get("cover_url") or "").strip() or None,
...
def normalize_isbn(raw_isbn: str) -> str | None:
    ...
    if len(compact) not in {10, 13}:
        raise ValueError("ISBN must be 10 or 13 characters (excluding spaces and hyphens).")
...
render_hero(
    ...
    "Keep the form simple, import details from Open Library when useful, and save with one clear action.",
    kicker="Book Form",
)
...
with st.expander("Import from Open Library", expanded=st.session_state.get("openlibrary_expanded", False)):
                ...
                if compact_layout:
                    ...
                    if st.button(
                        ...
                        try:
                            ...
                            merged_detail = merge_openlibrary_import_data(result, detail, isbn)
                            merged_detail["genre"] = merged_detail.get("genre") or st.session_state.get(
                                FORM_FIELDS["genre"], ""
                            )
                            ...
                            st.session_state["openlibrary_import_notice"] = (
                                "Book details imported. Continue below in the Add Book form."
                            )
                            st.rerun()
                        except RuntimeError as exc:
                            if show_openlibrary_timeout(str(exc)):
